...
class Actor(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.conv = nn.Sequential(
            nn.Linear(model_cfg.input_dim, model_cfg.nflts[0]),
            nn.BatchNorm1d(model_cfg.nflts[0]),
            nn.ReLU(),
            nn.Linear(model_cfg.nflts[0], model_cfg.nflts[1]),
            nn.ReLU()
            )
...
class Critic(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.model_cfg = model_cfg
        self.fc1 = nn.Linear(model_cfg.input_dim, model_cfg.nflts[0])
...
class Model(object):
    ...
    def __init__(self, model_cfg, critic=False, target=False, trainable=True):
        """Initialize parameters and build model.
        Params
        ======
    ...
    def train(self, inputs, actions, q_targets_next, dones, rewards, gamma):
        ...
        q_expected = self.net(inputs, actions)
    ...
    def predict(self, state, actions=None, detach=False, cpu_mem=False):
