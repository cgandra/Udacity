	...
	void stepHighway(double egoVelocity, long long timestamp, int frame_per_sec, pcl::visualization::PCLVisualizer::Ptr& viewer)
	{
		...
		for (int i = 0; i < traffic.size(); i++)
		{
			...
			if(trackCars[i])
			{
				...
				markerl = tools.lidarSense(traffic[i], viewer, timestamp, visualize_lidar);
				markerr = tools.radarSense(traffic[i], egoCar, viewer, timestamp, visualize_radar);
...
#ifdef LOG_MEAS
				std::cout << "L" << "\t" << timestamp << "\t" << markerl.x << "\t" << markerl.y << std::endl;
				std::cout << "R" << "\t" << timestamp << "\t" << markerr.rho << "\t" << markerr.phi << "\t" << markerr.rho_dot << std::endl;
#endif
#ifdef LOG
				std::cout << "gt" << "\t" << timestamp << "\t" << gt(0) << "\t" << gt(1) << "\t" << gt(2) << "\t" << gt(3) << "\t" << traffic[i].velocity << "\t" << traffic[i].angle << std::endl;
				std::cout << "es" << "\t" << timestamp << "\t" << estimate(0) << "\t" << estimate(1) << "\t" << estimate(2) << "\t" << estimate(3) << "\t" << v << "\t" << yaw << "\t" << traffic[i].ukf.x_(4) << std::endl;
#endif
			}
		}
...
#ifdef LOG
		std::cout << "rmse" << "\t" << timestamp << "\t" << rmse[0] << "\t" << rmse[1] << "\t" << rmse[2] << "\t" << rmse[3] << std::endl;
#endif
