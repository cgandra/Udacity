...
void euclideanCluster(int i, const std::vector<std::vector<float>>& points, std::vector<bool> &processed, KdTree* tree, float distanceTol, std::vector<int> &cluster)
{
	processed[i] = true;
	cluster.push_back(i);
	...
	{
		if (!processed[id])
			euclideanCluster(id, points, processed, tree, distanceTol, cluster);
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int i = 0; i < points.size(); i++)
	{
		...
		euclideanCluster(i, points, processed, tree, distanceTol, cluster);
